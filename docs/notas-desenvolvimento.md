...
- **🔎 EM ABERTO (v0.85.1) — Listagem geral (`busca_andamento.asp`, categoria vinil) só rende lote
  em 1-2 páginas por chamada.** Achado ao investigar a casa "Miss leilões" ausente (ver
  changelog v0.85.1). `findlot` não acha lote de casas que sabidamente estão na categoria (Flavia
  Santos, Miss leilões); o app depende quase só do `galleryscan`, que também falha pra casas com
  mais de 1 página na galeria. Próximo passo: `step=pagedebug` (via `debug-cron.yml`) com
  `pages=1,2,3,40,last-1,last`, depois com `cookie=1` e com `delayMs=1500`, pra ver status/corpo
  real de cada página e escolher a correção (retry/backoff, sessão, pausa entre páginas).
