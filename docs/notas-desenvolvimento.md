...
5. **Encoding do JSON do catálogo (mojibake):** a `santavelharia.com.br` devolve o `DESCRICAO`
   com dupla-codificação UTF-8 (ex.: `descriÃ§Ã£o`, `nÃ£o`, `ruÃ­dos`). Avaliar se o
   `publicFetch` está decodificando a resposta como Latin-1 quando o corpo é UTF-8 (ou o
   servidor declara/serve charset trocado) e normalizar antes de gravar `artist`/`album`/`text`
   em `lot_sales`/`lot_condition` (afeta a leitura, não a captura de valor). Conferir se as casas
   já conhecidas (Discos Esquecidos/Catavento) também vêm afetadas.
