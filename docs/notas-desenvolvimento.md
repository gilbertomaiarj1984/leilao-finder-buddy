...
| v0.85.0      | **Fix de causa raiz** (achado ao investigar mais casas de lote ausente após v0.84.0 — Vinil 11, Dr Garage, Robson Gini/Trem das 7): `fetchCatalogData` (`leiloesbr-catalog.server.ts`, usada por `enrich`/`condition`/`sales`/`catalogdebug`) devolvia **0 itens pra QUALQUER leilão**, mesmo com o catálogo saudável e público — confirmado comparando com `step=catdebug` (fetch cru, sem Referer, achou `pecaHit`/`loteHit` normalmente no MESMO domínio+leilão que `catalogdebug` zerava). Causa: `fetchOnce` (`leiloesbr-auth.server.ts`, usada por `publicFetch`) manda por padrão `Referer: https://leiloesbr.com.br/default.asp` — correto pra URLs da própria LeilõesBR (listagem geral), mas as chamadas de catálogo batem no domínio da CASA (`tremdas7.com.br`, `abreucolecionismo.com.br`, etc.) — um Referer de origem cruzada é sinal clássico de bot pra proteções anti-scraping, e o servidor da casa respondia HTTP 200 com corpo vazio/diferente (nunca um erro, por isso nunca aparecia em nenhum log). Bug **antigo, não introduzido pela v0.84.0** — silencioso porque a listagem geral (que bate em `leiloesbr.com.br`, Referer correto) sempre funcionou, escondendo que qualquer coisa que lê o catálogo da CASA (nº de lote, estado Disco/Capa, captura de vendas) vinha sempre vazia. Fix: todo call site que busca uma URL do domínio da CASA passa a mandar `referer: <domínio da casa>/` em vez do padrão — `leiloesbr-catalog.server.ts` (`fetchCatalogJson`/`fetchCatalogHtml`), `leiloesbr-lot-details.server.ts` (`fetchOne`, sinal de "vendido"/próximo lance da `peca.asp`), `lot-sales.server.ts` (`fetchLotPageImage`, thumbnail de venda) e `leiloesbr-presencial.server.ts` (`fetchPresencialNow`, leitura de `idleilao`/`idsite`). Chamadas que já batem em `leiloesbr.com.br` (`BASE_URL`) ficam como estavam — Referer já correto. Validado em produção via `debug-cron.yml`/`step=catalogdebug` contra as 4 casas do achado antes de mesclar |
| v0.85.1      | **Diagnóstico** (achado do usuário, 2026-09-26: casa "Miss leilões" com 96 lotes no dia não aparece no app). Investigação em produção via `debug-cron.yml`: a Miss **está** na lista de galerias da categoria vinil (`step=galleries`, `ga=669`), mas `galleryscan` dela rende 0 e `findlot q=miss` varre as 85 páginas da listagem geral sem achar nada. Sinal maior: `findlot q=flavia` também não acha NENHUM lote da Flavia Santos nas 85 páginas (ela tem 39 no dia, que só entraram pelo `galleryscan`), e o log do `refresh.yml` mostra o mesmo padrão há dias — `chunk` só rende lote em 1-2 páginas por chamada (ex.: run `#182`: 56 + 126 lotes nas páginas 85–56, **0** nas páginas 55–1; run `#171`, 23/09: idem). Ou seja, a paginação da listagem geral está falhando/vindo vazia **em silêncio** (`catch { continue }` / `if (!lots.length) continue` em `scrapeVinylChunk`/`listGalleryAuctions`/`findLotDebug`), e o app vinha se sustentando quase só pelo `galleryscan`. Sem mecanismo confirmado ainda (hipóteses: bloqueio/limite de taxa com resposta 4xx rápida — 85 páginas em ~3s; paginação dependente da sessão ASP; resposta 200 sem cards). Adicionado: `publicFetchRaw` (`leiloesbr-auth.server.ts`, GET público sem retry e sem lançar — status, URL final, tempo, cookies, corpo); `debugListingPages`/`step=pagedebug&pages=1,2,last[&ga=][&tp=none][&cookie=1][&delayMs=]` (status/cards/parse/dias/casas por página + trecho do corpo quando não vem card); `chunk`/`findlot` passam a devolver `failedPages`/`emptyPages` e `galleryscan` devolve estatística por galeria (`galleries[]`: páginas, cards, mantidos, falhas) — o log do cron deixa de esconder isso. Próximo passo: rodar `pagedebug` em produção e corrigir a causa |
| v0.85.2      | **Fix de causa raiz** da casa "Miss leilões" ausente (e da maior parte da listagem geral sumindo em silêncio): `step=pagedebug` (v0.85.1) mostrou em produção que a `busca_andamento.asp` entrega as páginas 1 e 2 completas (~2,5s, 126 cards) e, a partir da 3ª requisição seguida, **HTTP 200 com corpo vazio** (0 bytes em ~5ms) — limite de taxa do servidor. Como é 200, `publicFetch` não lançava e `parseCards` só não achava card: `chunk` rendia 1-2 páginas por chamada há dias, e o app se sustentava quase só pelo `galleryscan` (que também só lia a 1ª página de cada galeria). Com 2s e com 0,8s de intervalo, todas as páginas vieram completas — inclusive os 96 lotes da Miss (páginas 79-80). Fix: `listingFetch` (fila única, 1s de intervalo mínimo, retry 4s/8s em corpo vazio) em `fetchPage`/`fetchPageSearch`; orçamento de tempo (`CHUNK_BUDGET_MS`=75s) em `scrapeVinylChunk` (cursor `nextPage` pela última página processada) e `scanGalleries` (só abre galeria nova com orçamento, `nextOffset` pelo que foi processado, corte duro dentro de `listGalleryAuctions`); `listGalleryAuctions` para após 2 páginas seguidas inteiras além da janela (listagem medida: página 1 = dia mais distante) e devolve `scanned`/`truncated`. Laço do `galleryscan` no `refresh.yml` e no botão "Atualizar tudo" de 25 → 70 voltas. A varredura geral continua completa (não voltou o corte precoce), agora ~3-4s por página |
