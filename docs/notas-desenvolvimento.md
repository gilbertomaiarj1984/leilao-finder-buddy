...
- **Ícone roxo "já tenho na Coleção"** (`LotCard`, só na **home** `index.tsx`): disco `Disc3`
  ...
  Casamento reaproveita o motor da Sondagem (`wantlist-match`): `ownedCands` (artista+álbum da
  coleção, ignora buckets Lote/Coletâneas/Não classificados) → `bestWantForLot` ≥ 80%; peça
  exata (`lot_id`) casa direto. Mapa `ownedById` memoizado; prop `owned` no `LotCard`, tooltip
  "Já tenho na Coleção: Artista Álbum".
...
| v0.23.0 | Home: **ícone roxo "já tenho na Coleção"** no card (abaixo da nota, à direita) quando o lote casa com `collection_items` — casamento por artista/álbum/ano via `wantlist-match` (≥80%) + peça exata por `lot_id` | — |
