...
| v0.69.28     | **Confirmado resolvido o bug do `step=aieval`, fechando de vez a investigação v0.69.15-27.** Motivo do relato do usuário ("muitos lotes sem avaliação e nota da IA"): o secret `APP_URL` já tinha sido trocado — as runs `#144`-`#147` do `refresh.yml` (checadas via API do GitHub Actions) mostram o `aieval` respondendo `200`, conectado de verdade em `143-95-214-240.sslip.io` (certificado `CN=143-95-214-240.sslip.io`), nenhuma reprodução do 500 em 4 rodadas seguidas. Removida a instrumentação temporária que não serve mais pra nada: header `X-Debug-Upstream` no `Caddyfile` (v0.69.21), `curl -v`/`-D` na chamada do `aieval` em `refresh.yml` (v0.69.26, revertido ao `call()` padrão) e o `pid`/`uptimeSec`/`hostname`/`hasDatabaseUrl` no corpo de erro 500 do `catch` de `handleCron` (`cron.server.ts`, v0.69.16); `health_uri` do Caddy (v0.69.21) **mantido** como mitigação estrutural de baixo custo. A causa real do "muitos lotes sem nota" não é mais um bug de infra: o `ai_mode` padrão é `"watched"` (econômico, ver seção "IA") — o cron automático só avalia lotes vigiados/com lance, o resto do catálogo fica sem nota até um "Analisar dia/casa" manual ou a troca do modo pra "Todos" na UI; usuário optou por manter `"watched"` |

## Pendências

**Produto / código (em aberto)**

- Nenhuma pendência de código em aberto no momento (a última, o 500 intermitente do
  `step=aieval`, foi confirmada resolvida na v0.69.28 — ver Histórico de versões).
