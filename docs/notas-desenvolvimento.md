...
| v0.48.3        | **Home: header enxuto + barra de filtros/dias/casas fixa; Coleção: filtro/seletor de visão fixo** — (1) home: removido o bloco "LeilõesBR / Garimpo de Vinil / descrição" do header (fica só e-mail+Sair e as ações existentes numa única linha fina); a barra de busca + abas de dias/Vigiados/Lances passou a viver dentro do MESMO `<Tabs>` que o conteúdo e ficou **fixa logo abaixo do header** (um único wrapper `sticky top-0`, header e barra de filtros colam juntos sem precisar calcular a altura um do outro); reduzido o espaço em branco entre a barra de dias e a barra de ações do dia (`py-8`→`pt-3 pb-8` no container do conteúdo). (2) a barra "Vigiados do dia/Lances do dia" (que já inclui os **botões de casa**) e as duas seções por dia dentro das abas globais **Vigiados**/**Lances** também eram `sticky top-0`, mas coincidiam com o `top:0` do header e ficavam escondidas atrás dele; `headerRef`+`ResizeObserver` mede a altura real do header ao vivo (dinâmica, pois a busca/abas pode quebrar linha) e usa esse valor como `top` inline nessas 3 barras. (3) Coleção: filtro de artista/busca e o seletor **Cards/Títulos** (`TabsList`) movidos para dentro do header sticky (mesma técnica), ficando fixos ao rolar a página | —       |
| v0.48.4        | **Fase 1 do plano de economia — corrige a causa raiz (egress do Supabase + Active CPU da Vercel)** — `reidentifyAllSales` não baixa mais `lot_sales`+`lot_ident` INTEIRAS a cada chamada: (1) o passo de identificação por IA (modo GLOBAL) agora pede ao banco só as vendas SEM linha em `lot_ident`, via nova RPC de anti-join `get_unidentified_lot_sales` (`supabase/migrations/20260914000000_reident_egress_fixes.sql`); lotes sem texto/`isDiscBundle` na janela são marcados "tentado" (`lot_ident` com álbum nulo) para não ocupar a janela de novo a cada chamada — sem isso o laço nunca chegaria a `done`. (2) a padronização de grafia (passo 2, ainda lê o histórico inteiro) usa `getAllLotSales({ withOrig: false })` — sem a coluna `orig_text` (a mais pesada por linha), que essa etapa não usa; a regravação das linhas que mudam agora OMITE `orig_text` do payload do upsert em vez de mandar `""` (bug que apagaria o texto original já gravado — `upsertLotSales` aceita `orig_text` opcional e o PostgREST só sobrescreve colunas presentes no corpo). (3) nova coluna `lot_sales.bundle` (calculada na captura a partir de `orig_text`) substitui a checagem `isDiscBundle(orig_text)` no Vinil Analytics: `getVinylSales` (`leiloesbr.functions.ts`) não pede mais `orig_text` ao navegador — vazamento secundário do plano. Novo cron step **`backfillbundle`** (`lot-sales.server.ts`/`cron.server.ts`) faz o backfill ÚNICO de `bundle` nas vendas gravadas antes da coluna existir — **rodar manualmente uma vez** (`/api/cron?step=backfillbundle`) após aplicar a migration. Laço do `reident` no `refresh.yml` reduzido de 60 para 15 iterações (converge em 1-2 na maioria dos casos). Ver `docs/economia-fase-1-egress-e-cpu.md`. **Pendente**: aplicar a migration no Supabase (SQL Editor), rodar o backfill uma vez, e reconferir os painéis de uso do Supabase/Vercel depois de ~1 semana | —       |
