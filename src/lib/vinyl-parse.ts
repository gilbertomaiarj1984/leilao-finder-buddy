...
// perfumaria, couro/bijuteria, livro/quadro/lata/brinquedo, prataria/utensílios antigos,
// documentos/manuscritos históricos…) — casas que vendem de tudo às vezes colocam esses itens
// na MESMA categoria "Disco de vinil" do site (ou o filtro `Tipo=129` não é honrado pela
// casa) e eles entram na nossa varredura/captura por não baterem em NENHUM formato de mídia
// bloqueado (CD/DVD/K7…). Cada item vira seu próprio "álbum" no Analytics (preço de uma coisa
// que não é disco). Lista por CATEGORIA de colecionismo, não por marca/evento — cresce
// conforme aparecem casos reais.
const NON_MEDIA_COLLECTIBLE_RE =
  /\b(ex-?libris|pin\s+de\s+lapela|broche|medalh(?:a|ao|oes|as)|moedas?|cedulas?|numismatic\w*|selos?\s+postais?|filatelic\w*|porta-?moedas|carteira\s+de\s+couro|cinto\s+de\s+couro|perfume|colonia|fragrancia|estatueta|porcelana|an(?:el|eis)|joias?|bijuteria|bijouteria|pulseiras?|colar(?:es)?|brincos?|pingentes?|correntes?\s+de\s+(?:ouro|prata)|relogios?\s+de\s+pulso|aliancas?|livros?|revistas?|quadros?|pinturas?|latas?|brinquedos?|miniaturas?|carrinhos?|bonec[oa]s?|salvas?|bandejas?|prataria|baixelas?|castical|casticais|talheres?|molheiras?|centro\s*de\s*mesa|rascunhos?|bilhetes?|manuscritos?|cartas?|brochuras?|paginas?|escritas?|opusculos?|folhetos?|panfletos?)\b/;
